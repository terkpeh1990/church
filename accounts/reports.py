...
def statement_of_accounts(request,pk):
    ...
    sub_account = Sub_Accounts.objects.filter(code__group='Revenue',all_transaction__type='Church')
    expense_sub_account = Sub_Accounts.objects.filter(code__group='Expense',all_transaction__type='Church')
    ...
    rev_cash_eq = All_Transaction.objects.filter(sub_code__code__group='Revenue',type='Church',account_period=pv.id)
    exp_cash_eq = All_Transaction.objects.filter(sub_code__code__group='Expense',type='Church',account_period=pv.id)
