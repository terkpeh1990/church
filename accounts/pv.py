...
def manage_pv(request):
    church = request.user.profile.church
    pv = Payment_Vouchers.objects.filter(church=church)
...
def pending_pv(request):
    church = request.user.profile.church
    pv = Payment_Vouchers.objects.filter(church=church,status = "Pending")
    template = 'accounts/manage_pv.html'
    context={
        'pv':pv,
    }
...
def approve(request,pk):
    pv = Payment_Vouchers.objects.get(id=pk)
    pv.status = "approved"
    Account_Payables.objects.create(transaction_date=pv.transaction_date,sub_code=pv.sub_account,description=pv.description,amount=pv.amount,reference=pv,church=pv.church,type=pv.type,account_period=pv.account_period)
    All_Transaction.objects.create(transaction_date=pv.transaction_date,sub_code=pv.sub_account,description=pv.description,amount=pv.amount,church=pv.church,type=pv.type,account_period=pv.account_period)
    General_Ledger.objects.create(transaction_date=pv.transaction_date,sub_code=pv.sub_account,description=pv.description,debit=pv.amount,church=pv.church,type=pv.type,transactionref=pv.id,account_period=pv.account_period)
    pv.save()
...
def void_pv(request,pk):
    ...
    General_Ledger.objects.create(transaction_date=pv.transaction_date,sub_code=pv.sub_account,description=pv.description + " "+ "has been made void" ,cedit=payables.amount,church=pv.church,transactionref=pv.id,account_period=pv.account_period)
    All_Transaction.objects.create(transaction_date=pv.transaction_date,sub_code=pv.sub_account,description=pv.description,amount=-payables.amount,church=pv.church,account_period=pv.account_period)
